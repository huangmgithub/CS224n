...
def minibatch_parse(sentences, model, batch_size):
    ...
    partial_parses = {PartialParse(sentence):i for i, sentence in enumerate(sentences)} # 将 parser 和序号做一个映射，方便按顺序添加依赖
    unfinished_parses = list(partial_parses.keys())
    while unfinished_parses:
        batch_parses = unfinished_parses[:batch_size]
        next_transitions = model.predict(batch_parses)
        for partial_parse, transition in zip(batch_parses, next_transitions):
            partial_parse.parse_step(transition)
            if len(partial_parse.buffer) == 0 and len(partial_parse.stack) == 1: # 一个句子完成解析
                print(partial_parse)
                dependencies[partial_parses[partial_parse]] = partial_parse.dependencies
